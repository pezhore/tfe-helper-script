...
WORKSPACE_ID=($(curl \
  ...
  https://tfe.local/api/v2/organizations/$ORG_NAME/workspaces/$WORKSPACE_NAME \
  | jq -r '.data.id'))
...
UPLOAD_URL=($(curl \
  ...
  https://tfe.local/api/v2/workspaces/$WORKSPACE_ID/configuration-versions \
  | jq -r '.data.attributes."upload-url"'))

# 5. Upload the Configuration Content File

curl \
  -k --header "Content-Type: application/octet-stream" \
  --request PUT \
